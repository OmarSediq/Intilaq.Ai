...
class InterviewFeedbackService(TraceableService):
    def __init__(self, validator: InterviewValidatorService, gemini_service: GeminiAIService , repo_answer : InterviewAnswerRepository):
    ...
    async def get_feedback(self, session_id: int, question_index: int, user_id: str):
        ...
        latest_answer = await self.repo_answer.find_latest_answer(session_id, user_id)
        ...
        await self.repo_answer.update_answer_feedback(session_id, user_id, question_index, {
            "feedback": feedback,
