...
class AuthService(TraceableService):
    def __init__(self, db: AsyncSession , user_repo : UserRepository ,token_service : TokenService  , refresh_token_service : RefreshTokenService):
        self.db = db
    ...
    async def login(self, request, response: Response):
        ...
        await self.refresh_token_service.store_refresh_token(str(user.id), refresh_token)
    ...
    async def logout(self, request: Request, response: Response):
        ...
        try:
            user_id = self.refresh_token_service.decode_refresh_token(token)["user_id"]
            if await self.refresh_token_service.get_stored_refresh_token(user_id) != token:
                return error_response(code=401, error_message="Invalid refresh token")
            await self.refresh_token_service.delete_refresh_token(user_id)
