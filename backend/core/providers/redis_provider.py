...
async def stream_xadd(
    ...
    *,
    ...
    if max_len:
        return await redis.xadd(stream_name, mapping, maxlen=max_len, approximate=approximate)
    return await redis.xadd(stream_name, mapping)


async def stream_xreadgroup(
    redis: Redis,
    group: str,
    consumer: str,
    ...
    *,
    ...
    return await redis.xreadgroup(group, consumer, streams, count=count, block=block)


async def stream_xack(redis: Redis, stream: str, group: str, message_id: str):
    return await redis.xack(stream, group, message_id)


# ---- Idempotency ----

async def claim_short(
    redis: Redis,
    key: str,
    value: str,
    ttl_seconds: int,
) -> bool:
    return await redis.set(key, value, nx=True, ex=ttl_seconds)


async def release_claim(redis: Redis, key: str):
